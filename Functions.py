...
def getTodayMeal():
    ...
    return getMeal(formattedDate)

def getTomorrowMeal():
    now = datetime.datetime.now()
    tomorrow = now + datetime.timedelta(days=1)
    formattedDate = tomorrow.strftime("%Y%m%d")

    return getMeal(formattedDate)

def getMeal(YYYYMMDD):
    url = 'https://school.ch1ck.xyz/api/meal?edu=J10&school=7530892&date='  # &date=20211220 와 같이 &date=YYYYMMDD 형식으로 리퀘 보낼 경우, 특정 일자 급식 정보 받아올 수 있음.
    url += YYYYMMDD
    ...
    try:
        ...
        meal_message = f'{YYYYMMDD}의 {code}은 놀랍게도 {calorie} 이에요! \n\n'
        ...
        for i in range(len(school_menu[0]['menu'])): # 향상된 for문 
            ...
            allergy_all = ", ".join(school_menu[0]['menu'][i]['allergy']) # 각 메뉴당 알러지 정보 추가.
